...
- Module callbacks receive the author host consistently: `host.mutation.patch(function(plan, host, store) ... end)` and `onSettingsCommitted(host, store, commit)`. Draw callbacks now receive `drawTab(draw, data, actions, services)` and `drawQuickContent(draw, data, actions, services)`, where `draw` contains `imgui` plus bound `widgets` / `nav`.
