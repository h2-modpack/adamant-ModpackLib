# ModpackLib Docs

Choose the path that matches the work.

## Module Authors

- [module-authors/GETTING_STARTED.md](module-authors/GETTING_STARTED.md)
- [module-authors/MODULE_AUTHORING.md](module-authors/MODULE_AUTHORING.md)
- [module-authors/DRAW_LIFECYCLE.md](module-authors/DRAW_LIFECYCLE.md)
- [module-authors/capabilities/README.md](module-authors/capabilities/README.md)
- [module-authors/UI_PERFORMANCE.md](module-authors/UI_PERFORMANCE.md)
- [../API.md](../API.md)

Use this path when building a module or modpack feature on top of Lib.

## Modpack Authors

- [modpack-authors/COORDINATOR_GUIDE.md](modpack-authors/COORDINATOR_GUIDE.md)
- [modpack-authors/QUICK_SETUP.md](modpack-authors/QUICK_SETUP.md)
- [modpack-authors/HASH_PROFILE_ABI.md](modpack-authors/HASH_PROFILE_ABI.md)

Use this path when building or maintaining a coordinator package that calls
`lib.modpack`.

## Lib Contributors

- [lib-contributors/LIB_INTERNALS.md](lib-contributors/LIB_INTERNALS.md)
- [lib-contributors/HOT_RELOAD_ARCHITECTURE.md](lib-contributors/HOT_RELOAD_ARCHITECTURE.md)
- [lib-contributors/CONTROL_ERGONOMICS.md](lib-contributors/CONTROL_ERGONOMICS.md)
- [lib-contributors/TESTING.md](lib-contributors/TESTING.md)
- [lib-contributors/future-ideas/README.md](lib-contributors/future-ideas/README.md)
- [../CONTRIBUTING.md](../CONTRIBUTING.md)

Use this path when changing Lib itself.

## References

- [references/KNOWN_LIMITATIONS.md](references/KNOWN_LIMITATIONS.md)
- [references/IMGUI_LUA_REFERENCE.md](references/IMGUI_LUA_REFERENCE.md)
- [references/RELOAD_MODUTIL_CHALK_REFERENCE.md](references/RELOAD_MODUTIL_CHALK_REFERENCE.md)

Use these when you need platform behavior, accepted constraints, or binding details.

## Rejected Directions

- [rejected-directions/DECLARATIVE_FIELD_UI.md](rejected-directions/DECLARATIVE_FIELD_UI.md)

These record explored designs that should not be revived without a deliberate
architecture review.
